...
import {
  getFavoriteData,
  getFavoriteLeagueAndDates,
  getFavoriteGameIds,
  getFavoriteByGameIds,
} from "/data/favorite-data";
...
function OddsHome(props) {
  ...
  const updateOddsTables = (gameId, action) => {
    if (props.isFavorites && action === "delete") {
      let output = oddsTables
        .map((item) => {
          return {
            league: item.league,
            oddsTableModel: {
              gameRows: item.oddsTableModel.gameRows.filter((row) => row.gameView.gameId != gameId),
  ...
  useEffect(() => {
    if (props.isFavorites) setFavorites(props.oddsTables);
  ...
  let headerSet = false;
  let pageTitle = `${props.league.replace("-", " ")} Odds, Lines and Spreads ${
    props.stringScope !== "" ? "-" : ""
  } ${props.stringScope}${
    query.date !== undefined ? ` for ${getTimeHelpers.getDate(query.date, "titleOption")}` : ""
  }`;
  if (props.isFavorites) pageTitle = "Favorites";
    ...
    <>
      <Head>
        ...
        <meta
          name="description"
          content={
            props.bettingArticle?.metaDescription ??
            "Free Betting Odds and line movements in realtime at Sportsbookreview.com. Check out SBR's live odds for all major sports."
          }
        />
        <meta property="og:type" content="website" />
        <meta
          property="og:title"
          content={`${props.bettingArticle?.metaTitle ?? "Betting Odds, Lines and Spreads"}`}
        />
        <meta
        ...
        <meta
          property="og:image"
          content="https://img.sportsbookreview.com/images/sbr-open-graph.png"
        />
        <meta
          property="og:image:alt"
          content="The SBR logo with the bull at the top of the SBR letters"
        />
        ...
        {props.breadcrumbListObject !== ""
          ? parse(
              '<script type="application/ld+json">' +
                JSON.stringify(props.breadcrumbListObject) +
                "</script>"
            )
          : ""}
      ...
      <div id={props.header} className="container content bckg-white">
        <PageControls
          ...
          handler={setOddsFormat}
          isBreakpoint={breakpoint}
          scopes={query}
          breadcrumbObject={props.breadcrumbListObject}
          pageName={props.isFavorites ? "Favorite Events" : undefined}
        />
        {oddsTables.length === 0 ||
        (oddsTables.length === 1 && oddsTables[0].oddsTableModel.gameRows.length == 0) ? (
          props.isFavorites ? (
            <div className="noOdds">
              <h2>No Favorites Yet!</h2>
              <p className="flex-centered">
                Click on the button{" "}
                <span className={`flex-centered ${styleFavorite.iconFavorite}`}>
                  <i className="sbr-icon-star-icon flex-centered"></i>
                </span>{" "}
                on any game page to add a favorite.
              </p>
            </div>
          ) : (
            <div className="noOdds">
              <h2>No odds available at this time for this league</h2>
            </div>
          )
        ) : (
          <>
            ...
            <div id="leagues">
              {oddsTables.map((row, index) =>
                row.oddsTableModel.gameRows.length > 0 ? (
                  <section
                    key={index}
                    id={`section-${row.league.toLowerCase()}`}
                  ...
                  >
                    <OddsTable
                      ...
                      displayFootballHeader={
                        props.footballLeaugeLanding ? (index === 0 ? true : false) : true
                      }
                    />
                    {(headerSet = true)}
                  </section>
      ...
      {isBreakpoint ? <FooterFlyoutMobile favoriteCount={favoriteCount} /> : null}

      {/* <Footer region={props.header} /> */}
      <StellarFooter label="Betting-Odds Stellat Footer" />
    </>
  );
}
...
export async function getServerSideProps(context) {
  ...
  if (
    (context.params?.league && leagueObject === null) ||
    (context.params?.oddsType != null && context.params?.oddsScope === undefined)
  ) {
    return {
      notFound: true,
    };
  ...
  if (!!context.query.league && !context.query.date) pastFallback = true;
  ...
  // Show games within the next seven days for football leagues ONLY on the league landing page
    ...
    league !== "Betting" &&
    (league.toLowerCase() === "ncaaf" || league.toLowerCase() == "nfl") &&
    !hasDateQuery
  ) {
  ...
  let oddsTables = await Promise.all(
    leagueDataArr.map(async (item) => {
      ...
      if (!isFavorites) {
        ...
        if (item.league === "MLB" || item.league === "NHL")
          oddsViewDefault = "MoneyLineDataOpeningAndLatestOddsDataView";
        if (isSoccerLeague(item.league))
          oddsViewDefault = "ThreeWayDataOpeningAndLatestOddsDataView";
      }
      var oddsView = oddsViewObject?.viewtype ?? oddsViewDefault;

      if (
        isFavorites &&
        isSoccerLeague(item.league) &&
        oddsView === "MoneyLineDataOpeningAndLatestOddsDataView"
      ) {
      ...
      var gameIds;
      if (isFavorites) {
        gameIds = getFavoriteGameIds(item.league, item.date, {
          req: context.req,
          res: context.res,
        });
      }
      ...
      if (league === "Betting") gameIds = gameIds.slice(0, 11);
      ...
      const oddsTableModel = buildOddsTable(
        oddsViews,
        gameViews,
        sportsbooks,
        liveScoreViews,
  ...
  oddsTables = oddsTables.filter(
    (odds) => !!odds.oddsTableModel && odds.oddsTableModel.gameRows.length > 0
  );
  const leagueInfo = allLeagueData.find((leagues) => leagues.leaguename === league);
  const breadcrumbListObject = breadcrumbObject(
    league,
    context.req,
    context.resolvedUrl,
    leagueInfo
  ...
  const sportListObject = getTimeHelpers.isBeforeToday(context.query.date)
    ? ""
    : sportsListEvent(oddsTables);

  const pathnameArr = context.resolvedUrl.substring(1).split("/");
  const slug = (
    league != "Betting" && pathnameArr.length > 1 ? pathnameArr[1] : pathnameArr[0]
  ).split("?")[0];
  const urlPath = league != "Betting" && pathnameArr.length > 1 ? pathnameArr[0] : null;
  const bettingArticle = !isFavorites
    ? await getBettingArticle(slug, urlPath, leagueObject?.leaguename, context.res)
    : "";
  const schemaFAQ =
    bettingArticle?.faqs?.questions && bettingArticle.faqs.questions.length > 0
      ? await faqList(bettingArticle.faqs)
      : "";
...
function getLiveViewType(value) {
  ...
  switch (league) {
    case "NHL":
      return "HockeyLivescoreDetailsDataView";
    case "NFL":
    case "CFL":
    case "NCAAF":
      return "FootballLivescoreDetailsDataView";
    case "MLB":
      return "BaseballLivescoreDetailsDataView";
    case "NBA":
    case "WNBA":
    case "NCAAB":
      return "BasketballLivescoreDetailsDataView";
    case "Soccer":
      return "SoccerLivescoreDetailsDataView";
